...
// 投稿藝廊（內頁） https://glowart-backend.onrender.com/creator/:id
router.get('/creator/:id', async function (req, res) {
    const { id } = req.params;
    try {
        // 並行查詢
        const [exhibition, categories, exhibitionCategories] = await Promise.all([
            pool.query(`SELECT * FROM Exhibition WHERE id = $1 AND is_submission = true`, [id]),
            pool.query(`
                SELECT c.* 
                FROM category c
                JOIN Exhibition_Category ec ON c.id = ec.category_id
                JOIN Exhibition e ON ec.exhibition_id = e.id
                WHERE ec.exhibition_id = $1 AND e.is_submission = true
            `, [id]),
            pool.query(`
                SELECT * 
...
// 愛心路由
router.post('/creator/:id/like', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
// 檢查是否已按愛心
router.get('/creator/:id/like-status', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
...
// 收藏路由（類似按讚路由）
router.post('/creator/:id/collect', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
...
// 檢查是否已按收藏
router.get('/creator/:id/collect-status', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
...
// 創作者提交展覽的推薦路由
router.get('/creator/:id/recommended', async (req, res) => {
    const { id } = req.params;
    ...
    if (!isSubmission) {
        return res.status(301).json({
            redirectUrl: `/exhibition/${exhibitionId}/recommended`,
            message: '該展覽已發佈'
        });
    }

    // 查詢推薦展覽
    const result = await findRecommendedExhibitions(exhibitionId, true);
